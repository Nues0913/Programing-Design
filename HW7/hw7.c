...
#include<math.h>
int options = 0;
int hardDriveOfFileName[21] = {0};
int hardDriveOfFileSize[21] = {0};
int fileAmount = 0;
int fileName;
int fileSize;
...
void rearrange(){
    ...
    for(int j = 0;j <= 20;j++){
        if(temp[j] != 0){
            ...
            hardDriveOfFileName[j] = 0;
        }
    }
    int temp2[21] = {0};
    i = 0;
    for(int j = 0;j <= 20;j++){
        if(hardDriveOfFileSize[j] != 0){
            ...
            case 1:
                ...
                if(fileAmount <= 19){   //20 is the redundant one
                    ...
                    for(int i = 0;i < k;i++){
                        ...
                        hardDriveOfFileName[bestCombinationIndex[i]] = 0;
                        hardDriveOfFileSize[bestCombinationIndex[i]] = 0;
                        if(i != k-1)printf(" ");
                    }
                    fileAmount -= k-1;
                    printf("\n");
